...
def make_timeseries_regressor(window_size, filter_length, nb_input_series=1, nb_outputs=1, nb_filter=4):
    ...
    model = Sequential((
        ...
        # Convolution1D(nb_filter=32, filter_length=5, activation='relu', input_shape=(window_size, nb_input_series)),
        Convolution1D(nb_filter=32, filter_length=5, activation='relu', input_shape=(150, nb_input_series)),
        Convolution1D(nb_filter=32, filter_length=5, activation='relu'),

        MaxPooling1D(),     # Downsample the output of convolution by 2X.
        
        Convolution1D(nb_filter=32, filter_length=4, activation='relu'),
        Convolution1D(nb_filter=32, filter_length=3, activation='relu'),
        MaxPooling1D(),
...
def evaluate_timeseries(timeseries, window_size):
    ...
    dataset.load()
    model.fit(dataset.train_images, dataset.train_labels, nb_epoch=1, batch_size=2, validation_data=(dataset.valid_images, dataset.valid_labels))
