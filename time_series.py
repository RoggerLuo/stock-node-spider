...
import numpy as np
from keras.layers import Convolution1D, Dense, MaxPooling1D, Flatten,Dropout,Activation
...
def make_timeseries_regressor(window_size, filter_length, nb_input_series=1, nb_outputs=2, nb_filter=4):
    """
    :Return: a Keras Model for predicting the next value in a timeseries 
    given a fixed-size lookback window of previous values.
    ...
    model = Sequential((
        ...
        Convolution1D(nb_filter=8, filter_length=3, activation='relu', input_shape=(150, nb_input_series)),
        Convolution1D(nb_filter=8, filter_length=4, activation='relu'),
        MaxPooling1D(),     # Downsample the output of convolution by 2X.
        
        Convolution1D(nb_filter=4, filter_length=3, activation='relu'),
        Convolution1D(nb_filter=4, filter_length=3, activation='relu'),
        MaxPooling1D(),
        
        Dropout(0.25),                                    #12 Dropout层

        Flatten(),
        Dense(30),
        Activation('relu'),
        # Dropout(0.5),                                    #12 Dropout层
...
def make_timeseries_instances(timeseries, label): #1000
    """
    Make input features and prediction targets from a `timeseries` for use in machine learning.

    :return: A tuple of `(X, y, q)`.  
    `X` are the inputs to a predictor, a 3D ndarray with shape
...
#    X = np.atleast_3d(np.array([timeseries[start:start + window_size] for start in range(0, timeseries.shape[0] - window_size)]))
    X = [timeseries] #意思是当成一次使用
    X = np.atleast_3d(np.array(X))

    # y = timeseries[window_size:]
    y = label
    # q = np.atleast_3d([timeseries[-window_size:]])
    q = np.atleast_3d([timeseries])
    return X, y, q


def evaluate_timeseries(timeseries, window_size):
    """
    Create a 1D CNN regressor to predict     the next value[对我来说就是 涨10%还是不涨]     in a `timeseries` 
    using the preceding `window_size` elements(使用之前的lookback元素，对我来说就是150)
    as input features and evaluate its performance.

    [等等，没错，用之前的150，  来预测未来20个工作日是否涨幅超过10%]
    [它是用 之前的150个，  来预测第 151个， ]
    只要把target变一下就好了
    

    :param ndarray timeseries: Timeseries data with time increasing down the rows (the leading dimension/axis).
    timeseries，就是一个一维数组，也可以看成是我的一个json文件

    :param int window_size: The number of previous timeseries values to use to predict the next.
    我的window_size = 150， 就是整个的长度


    它的timeseries总长度是 1000, window_size是5
    所以，一个一维数据可以测试200次

    而我，timerseries总长度150, window_size是150，一个json只能测试一次
    """
    ...
    model = make_timeseries_regressor(window_size=window_size, filter_length=filter_length, nb_input_series=nb_series, nb_outputs=2, nb_filter=nb_filter)
    ...
    model.fit(dataset.train_images, dataset.train_labels, nb_epoch=3, batch_size=2, validation_data=(dataset.valid_images, dataset.valid_labels))
...
def main():
    ...
    # timeseries 是从0到999的一维数组,window_size =50(lookback)往回看多少
